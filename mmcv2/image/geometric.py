...
def rescale_size(
    ...
    scale: float | int | tuple[int, int],
    return_scale: bool = False,
) -> tuple[int, int] | tuple[tuple[int, int], float]:
    ...
    if isinstance(scale, (float, int)):
        ...
        scale_factor = float(min(max_long_edge / max(h, w), max_short_edge / min(h, w)))
...
def bbox_scaling(bboxes: np.ndarray, scale: float, clip_shape: tuple[int, int] | None = None) -> np.ndarray:
...
def impad_to_multiple(img: np.ndarray, divisor: int, pad_val: float | list = 0) -> np.ndarray:
...
def _get_shear_matrix(magnitude: float, direction: str = "horizontal") -> np.ndarray:
    ...
    """
    if direction == "horizontal":
        return np.array([[1, magnitude, 0], [0, 1, 0]], dtype=np.float32)
    return np.array([[1, 0, 0], [magnitude, 1, 0]], dtype=np.float32)
...
def imshear(
    ...
    sheared = cv2.warpAffine(
        ...
        borderValue=border_values[:3],
        flags=cv2_interp_codes[interpolation],
    )
    return sheared


def _get_translate_matrix(offset: float, direction: str = "horizontal") -> np.ndarray:
    ...
    """
    if direction == "horizontal":
        return np.array([[1, 0, offset], [0, 1, 0]], dtype=np.float32)
    return np.array([[1, 0, 0], [0, 1, offset]], dtype=np.float32)
