...
def imnormalize(img: np.ndarray, mean: np.ndarray, std: np.ndarray, to_rgb: bool = True) -> np.ndarray:
...
def imnormalize_(img: np.ndarray, mean: np.ndarray, std: np.ndarray, to_rgb: bool = True) -> np.ndarray:
...
def imdenormalize(img: np.ndarray, mean: np.ndarray, std: np.ndarray, to_bgr: bool = True) -> np.ndarray:
    assert img.dtype != np.uint8
    mean = mean.reshape(1, -1).astype(np.float64)
...
def adjust_brightness(img, factor=1.0):
    ...
    # preserve precision in the weighted sum.
...
def adjust_contrast(img, factor=1.0):
    ...
    Args:
        ...
        factor (float): Same as :func:`mmcv2.adjust_brightness`.
