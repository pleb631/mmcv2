...
from mmcv2.utils import mkdir_or_exist
from mmcv2.utils.path import is_filepath
...
class BaseStorageBackend(metaclass=ABCMeta):
    ...
    def get_local_path(self, filepath: str | Path) -> AbstractContextManager[str | Path]:
        raise NotImplementedError
...
class HTTPBackend(BaseStorageBackend):
    ...
    def get(self, filepath):
        value_buf = urlopen(str(filepath)).read()
        return value_buf
...
class FileClient:
    ...
    Args:
        backend (str, optional): The storage backend type. Options are "disk"
            and "http". Default: None.
        prefix (str, optional): The prefix of the registered storage backend.
            Options are "http" and "https". Default: None.
    ...
    Examples:
        ...
        >>> file_client = FileClient(backend='disk', prefix='http')
    ...
    _prefix_to_backends: dict[str, type[BaseStorageBackend]] = {
        "http": HTTPBackend,
        "https": HTTPBackend,
    }

    _instances: dict[str, "FileClient"] = {}
    ...
    def __new__(cls, backend=None, prefix=None, **kwargs):
        ...
        if arg_key in cls._instances:
            ...
            if backend is not None:
                ...
                _instance.client = cls._prefix_to_backends[cast(str, prefix)](**kwargs)
    ...
    def parse_uri_prefix(uri: str | Path) -> str | None:
        ...
        Examples:
            >>> FileClient.parse_uri_prefix('https://example.com/file.txt')
            'https'
    ...
    def infer_client(
        ...
        if file_client_args is None:
            file_prefix = cls.parse_uri_prefix(cast(str | Path, uri))
    ...
    def get_local_path(self, filepath: str | Path) -> Generator[str | Path, None, None]:
        ...
        Examples:
            >>> file_client = FileClient(backend='disk')
            >>> with file_client.get_local_path('/path/to/abc.jpg') as path:
            ...     # do something here
