...
from mmcv2.utils import Registry, build_from_cfg, get_logger, print_log

INITIALIZERS = Registry("initializer")


def _parameter(module: nn.Module, name: str) -> Tensor | None:
    """Return a tensor parameter exposed by a module, if present."""
...
def update_init_info(module: nn.Module, init_info: str) -> None:
    ...
    """
    params_init_info = cast(
        dict[nn.Parameter, dict[str, str | Tensor]],
        getattr(module, "_params_init_info"),
    )
    for name, param in module.named_parameters():
        assert param in params_init_info, (
            f"Find a new :obj:`Parameter` named `{name}` during executing the `init_weights` of `{module.__class__.__name__}`. Please do not add or replace parameters during executing `init_weights`."
        )
        # The parameter has been changed during executing the
        # `init_weights` of module
        mean_value = param.detach().mean()
        if params_init_info[param]["tmp_mean_value"] != mean_value:
            params_init_info[param]["init_info"] = init_info
            params_init_info[param]["tmp_mean_value"] = mean_value


def constant_init(module: nn.Module, val: float, bias: float = 0) -> None:
    if (weight := _parameter(module, "weight")) is not None:
        nn.init.constant_(weight, val)
    if (module_bias := _parameter(module, "bias")) is not None:
        nn.init.constant_(module_bias, bias)
...
def trunc_normal_init(
    ...
    if (weight := _parameter(module, "weight")) is not None:
        trunc_normal_(weight, mean, std, a, b)
    if (module_bias := _parameter(module, "bias")) is not None:
        nn.init.constant_(module_bias, bias)
...
class KaimingInit(BaseInit):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(**kwargs)
        self.a = a
        self.mode: Literal["fan_in", "fan_out"] = mode
        self.nonlinearity: Literal["relu", "leaky_relu"] = nonlinearity
        self.distribution = distribution
...
def _initialize_override(module: nn.Module, override: dict | list, cfg: dict) -> None:
...
def initialize(module: nn.Module, init_cfg: dict | list[dict]) -> None:
    ...
    Args:
        ...
        init_cfg (dict | list[dict]): initialization configuration dict to
            define initializer. Six initializers are provided:
    ...
    Example:
        ...
        >>> init_cfg = dict(type='Pretrained',
                checkpoint='/path/to/foonet.pth')
        >>> initialize(model, init_cfg)

        >>> # Initialize weights of a sub-module with the specific part of
        >>> # a pretrained model by using "prefix".
        >>> url = 'https://example.com/mmdetection/v2.0/retinanet/'\
