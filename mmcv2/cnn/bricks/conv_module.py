...
class ConvModule(nn.Module):
    ...
    def __init__(
        ...
        if self.with_activation:
            act_cfg_ = cast(dict, act_cfg).copy()
    ...
    def norm(self) -> nn.Module | None:
        """Return the normalization layer, if one is configured."""
        if self.norm_name:
    ...
    def init_weights(self):
        ...
        if self.with_norm:
            norm_layer = cast(nn.Module, self.norm)
            constant_init(norm_layer, 1, bias=0)

    def forward(self, x: torch.Tensor, activate: bool = True, norm: bool = True) -> torch.Tensor:
        """Apply convolution, optional normalization, and activation."""
        for layer in self.order:
            if layer == "conv":
                if self.with_explicit_padding:
                    x = cast(nn.Module, self.padding_layer)(x)
                x = self.conv(x)
            elif layer == "norm" and norm and self.with_norm:
                norm_layer = cast(nn.Module, self.norm)
                x = norm_layer(x)
            elif layer == "act" and activate and self.with_activation:
                activation_layer = cast(nn.Module, getattr(self, "activate"))
