...
class _NonLocalNd(nn.Module, metaclass=ABCMeta):
    ...
    def init_weights(self, std: float = 0.01, zeros_init: bool = True) -> None:
        """Initialize the output projection of the non-local block."""
        if self.mode != "gaussian":
        ...
        if zeros_init:
            if self.conv_out.norm_cfg is None:
                ...
                norm_layer = cast(nn.Module, self.conv_out.norm)
                constant_init(norm_layer, 0)
        else:
            if self.conv_out.norm_cfg is None:
                normal_init(self.conv_out.conv, std=std)
            else:
                norm_layer = cast(nn.Module, self.conv_out.norm)
                normal_init(norm_layer, std=std)

    def gaussian(self, theta_x: torch.Tensor, phi_x: torch.Tensor) -> torch.Tensor:
        """Compute Gaussian pairwise affinities."""
    ...
    def embedded_gaussian(self, theta_x: torch.Tensor, phi_x: torch.Tensor) -> torch.Tensor:
        """Compute embedded-Gaussian pairwise affinities."""
        # NonLocal1d pairwise_weight: [N, H, H]
        # NonLocal2d pairwise_weight: [N, HxW, HxW]
        # NonLocal3d pairwise_weight: [N, TxHxW, TxHxW]
        pairwise_weight = torch.matmul(theta_x, phi_x)
    ...
    def dot_product(self, theta_x: torch.Tensor, phi_x: torch.Tensor) -> torch.Tensor:
        """Compute scaled dot-product pairwise affinities."""
        # NonLocal1d pairwise_weight: [N, H, H]
        # NonLocal2d pairwise_weight: [N, HxW, HxW]
        # NonLocal3d pairwise_weight: [N, TxHxW, TxHxW]
        pairwise_weight = torch.matmul(theta_x, phi_x)
    ...
    def concatenation(self, theta_x: torch.Tensor, phi_x: torch.Tensor) -> torch.Tensor:
        """Compute concatenation-based pairwise affinities."""
        # NonLocal1d pairwise_weight: [N, H, H]
        # NonLocal2d pairwise_weight: [N, HxW, HxW]
        # NonLocal3d pairwise_weight: [N, TxHxW, TxHxW]
        h = theta_x.size(2)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply non-local attention to a feature tensor."""
