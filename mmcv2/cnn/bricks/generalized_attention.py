...
class GeneralizedAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.spatial_range >= 0:
            ...
            local_constraint_map = np.ones((max_len, max_len, max_len_kv, max_len_kv), dtype=int)
            ...
            self.register_buffer(
                "local_constraint_map",
                torch.from_numpy(local_constraint_map).to(torch.bool),
            )
    ...
    def get_position_embedding(
        ...
        """Build relative position embeddings for query and key locations."""
        h_idxs = torch.arange(h, device=device, dtype=dtype)
        h_idxs = h_idxs.view((h, 1)) * q_stride

        w_idxs = torch.arange(w, device=device, dtype=dtype)
        w_idxs = w_idxs.view((w, 1)) * q_stride

        h_kv_idxs = torch.arange(h_kv, device=device, dtype=dtype)
        h_kv_idxs = h_kv_idxs.view((h_kv, 1)) * kv_stride

        w_kv_idxs = torch.arange(w_kv, device=device, dtype=dtype)
        ...
        feat_range = torch.arange(feat_dim // 4, device=device, dtype=dtype)

        dim_mat = torch.tensor(wave_length, device=device, dtype=dtype)
    ...
    def forward(self, x_input: torch.Tensor) -> torch.Tensor:
        """Apply generalized attention to a feature map."""
        ...
        if self.spatial_range >= 0:
            cur_local_constraint_map = (
                cast(torch.Tensor, self.local_constraint_map)[:h, :w, :h_kv, :w_kv]
                .contiguous()
                .view(1, 1, h * w, h_kv * w_kv)
            )

            energy = energy.masked_fill_(cur_local_constraint_map.to(dtype=torch.bool), float("-inf"))
