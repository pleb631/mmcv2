...
class MomentumUpdaterHook(Hook):
    def __init__(
        ...
        self.base_momentum: list | dict = []
        # expected momentum if no warming up is performed
        self.regular_momentum: list | dict = []
    ...
    def get_regular_momentum(self, runner) -> list | dict[str, list]:
        if isinstance(runner.optimizer, dict):
    ...
    def get_warmup_momentum(self, cur_iters: int) -> list[float] | dict[str, list[float]]:
        ...
        def _get_warmup_momentum(cur_iters, regular_momentum):
            if self.warmup == "constant":
                ...
                raise ValueError(
                    f'Expected values of `self.warmup` to be "constant", "linear", or "exp", got {self.warmup}'
                )
...
class CosineAnnealingMomentumUpdaterHook(MomentumUpdaterHook):
    ...
    def __init__(
        self,
        min_momentum: float | None = None,
        min_momentum_ratio: float | None = None,
        **kwargs,
    ):
...
class LinearAnnealingMomentumUpdaterHook(MomentumUpdaterHook):
    ...
    def get_momentum(self, runner, base_momentum: float) -> float:
        ...
        if self.min_momentum_ratio is not None:
            ...
            assert self.min_momentum is not None
            target_momentum = self.min_momentum
        return annealing_linear(base_momentum, target_momentum, progress / max_progress)
...
class CyclicMomentumUpdaterHook(MomentumUpdaterHook):
    ...
    def __init__(
        self,
        by_epoch: bool = False,
        target_ratio: tuple[float, float] = (0.85 / 0.95, 1.0),
        cyclic_times: int = 1,
    ...
    def get_momentum(self, runner, base_momentum: float) -> float:
        ...
        raise RuntimeError("The method should return in the for-loop and should not be executed until this")
...
class OneCycleMomentumUpdaterHook(MomentumUpdaterHook):
    ...
    def __init__(
        self,
        base_momentum: float | list | dict = 0.85,
        max_momentum: float | list | dict = 0.95,
        pct_start: float = 0.3,
        anneal_strategy: str = "cos",
