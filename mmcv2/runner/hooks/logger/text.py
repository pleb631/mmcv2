...
class TextLoggerHook(LoggerHook):
    ...
    def __init__(
        ...
        if out_dir is None and backend_args is not None:
            raise ValueError('backend_args should be "None" when `out_dir` is not specified.')
        ...
        if self.out_dir is not None:
            self.backend = get_file_backend(self.out_dir, backend_args=backend_args)
    ...
    def before_run(self, runner) -> None:
        ...
        if self.out_dir is not None:
            ...
            runner.logger.info(
                f"Text logs will be saved to {self.out_dir} by {self.backend.name} after the training process."
            )

        self.start_iter = runner.iter
        self.json_log_path = osp.join(runner.work_dir, f"{runner.timestamp}.log.json")
        if runner.meta is not None:
            # Checkpoint-only RNG tensors cannot be represented in JSON logs.
            public_meta = {
                key: value
                for key, value in runner.meta.items()
                if key not in {"rng_states", "iter_loaders", "workflow_cursor"}
            }
            self._dump_log(public_meta, runner)

    def _get_max_memory(self, runner) -> int:
        accelerator = torch.accelerator.current_accelerator(check_available=True)
        if accelerator is None:
            return 0
        output_device = getattr(runner.model, "output_device", None)
        device = (
            torch.device(accelerator.type, output_device)
            if isinstance(output_device, int)
            else output_device or accelerator
        )
        mem = torch.accelerator.max_memory_allocated(device)
    ...
    def _log_info(self, log_dict: dict, runner) -> None:
        ...
        for name, val in log_dict.items():
            # Metadata and timing fields are formatted above.
    ...
    def log(self, runner) -> OrderedDict:
        ...
        log_dict: OrderedDict[str, Any] = OrderedDict(
    ...
    def after_run(self, runner) -> None:
        ...
        if self.out_dir is not None:
            for filename in scandir(runner.work_dir, self.out_suffix, True):
                ...
                out_filepath = self.backend.join_path(self.out_dir, filename)
                with open(local_filepath) as f:
                    self.backend.put_text(f.read(), out_filepath)
