    ...
    def before_run(self, runner):
        ...
        if callable(self.on_trace_ready):  # handler
            ...
            if trace_type == "log_trace":
                ...
                try:
                    import torch_tb_profiler  # noqa: F401  # pyright: ignore[reportMissingImports]
                except ImportError:
                    raise ImportError('please run "pip install torch-tb-profiler" to install torch_tb_profiler')
                if "dir_name" not in trace_cfg:
                    trace_cfg["dir_name"] = osp.join(runner.work_dir, "tf_tracing_logs")
                elif not osp.isabs(trace_cfg["dir_name"]):
                    trace_cfg["dir_name"] = osp.join(runner.work_dir, trace_cfg["dir_name"])
                runner.logger.info(f"tracing files of ProfilerHook will be saved to {trace_cfg['dir_name']}.")
