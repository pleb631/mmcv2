...
from mmcv2.parallel import is_module_wrapper
...
class EMAHook(Hook):
    ...
    def before_run(self, runner):
        ...
        for name, value in self.model_parameters.items():
            ...
            model.register_buffer(buffer_name, value.detach().clone())
    ...
    def after_train_iter(self, runner):
        ...
        for name, parameter in self.model_parameters.items():
            ...
            buffer_parameter.lerp_(parameter.detach(), momentum)
    ...
    def _swap_ema_parameters(self):
        ...
        for name, value in self.model_parameters.items():
            temp = value.detach().clone()
            ema_buffer = self.model_buffers[self.param_ema_buffer[name]]
            value.copy_(ema_buffer)
            ema_buffer.copy_(temp)
