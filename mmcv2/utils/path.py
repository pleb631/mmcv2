...
def is_filepath(x):
    """Return whether ``x`` can represent a filesystem path."""
    return isinstance(x, (str, Path))


def check_file_exist(filename, msg_tmpl='file "{}" does not exist'):
    """Raise ``FileNotFoundError`` unless ``filename`` is a file."""
...
def symlink(src, dst, overwrite=True, **kwargs):
    """Create a symbolic link, optionally replacing an existing link."""
