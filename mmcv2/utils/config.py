...
class ConfigDict(dict):
    """Dictionary with recursive attribute access for configuration data."""
...
class Config:
    ...
    Example:
        ...
        >>> cfg.filename
        "/home/kchen/projects/mmcv2/tests/data/config/a.py"
        >>> cfg.item4
        'test'
        >>> cfg
        "Config [path: /home/kchen/projects/mmcv2/tests/data/config/a.py]: "
    ...
    def fromfile(filename, use_predefined_variables=True, import_custom_modules=True):
        ...
        cfg_data = cast(dict[str, Any], cfg_dict)
        if import_custom_modules and cfg_data.get("custom_imports", None):
            import_modules_from_strings(**cast(dict[str, Any], cfg_data["custom_imports"]))
        return Config(cfg_data, cfg_text=cfg_text, filename=filename)
    ...
    def fromstring(cfg_str, file_format):
        ...
        if file_format != ".py" and "dict(" in cfg_str:
            # Python-style dictionaries can also occur inside JSON/YAML strings,
            # so the parser remains authoritative for the selected format.
            pass
    ...
    def merge_from_dict(self, options, allow_list_keys=True):
        ...
        super().__setattr__(
            "_cfg_dict",
            ConfigDict(Config._merge_a_into_b(option_cfg_dict, cfg_dict, allow_list_keys=allow_list_keys)),
        )
...
class DictAction(Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        options = {}
        for kv in cast(list[str], values):
