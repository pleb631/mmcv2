...
def test_assert_dict_contains_subset():
    ...
    assert mmcv2.assert_dict_contains_subset(dict_obj, expected_subset)

    # case 2
    expected_subset = {"a": "test1", "b": 2, "c": (6, 4)}
    assert not mmcv2.assert_dict_contains_subset(dict_obj, expected_subset)
...
def test_assert_attrs_equal():
    ...
    # case 1
    assert mmcv2.assert_attrs_equal(TestExample, {"a": 1, "b": ("wvi", 3), "c": [4.5, 3.14]})

    # case 2
    assert not mmcv2.assert_attrs_equal(TestExample, {"a": 1, "b": ("wvi", 3), "c": [4.5, 3.14, 2]})

    # case 3
    assert not mmcv2.assert_attrs_equal(TestExample, {"bc": 54, "c": [4.5, 3.14]})

    # case 4
    assert mmcv2.assert_attrs_equal(TestExample, {"b": ("wvi", 3), "test_func": TestExample.test_func})
    ...
    if torch is not None:
        ...
        # case 5
        assert mmcv2.assert_attrs_equal(TestExample, {"a": torch.tensor([1]), "b": torch.tensor([4, 5])})

        # case 6
        assert not mmcv2.assert_attrs_equal(TestExample, {"a": torch.tensor([1]), "b": torch.tensor([4, 6])})


assert_dict_has_keys_data_1 = [({"res_layer": 1, "norm_layer": 2, "dense_layer": 3})]
assert_dict_has_keys_data_2 = [
    (["res_layer", "dense_layer"], True),
...
def test_assert_dict_has_keys(obj, expected_keys, ret_value):
    assert mmcv2.assert_dict_has_keys(obj, expected_keys) == ret_value
...
def test_assert_keys_equal(result_keys, target_keys, ret_value):
    assert mmcv2.assert_keys_equal(result_keys, target_keys) == ret_value


@pytest.mark.skipif(torch is None, reason="requires torch library")
def test_assert_is_norm_layer():
    # case 1
    assert not mmcv2.assert_is_norm_layer(nn.Conv3d(3, 64, 3))

    # case 2
    assert mmcv2.assert_is_norm_layer(nn.BatchNorm3d(128))

    # case 3
    assert mmcv2.assert_is_norm_layer(nn.GroupNorm(8, 64))

    # case 4
    assert not mmcv2.assert_is_norm_layer(nn.Sigmoid())
...
def test_assert_params_all_zeros():
    ...
    assert mmcv2.assert_params_all_zeros(demo_module)

    nn.init.xavier_normal_(demo_module.weight)
    nn.init.constant_(demo_module.bias, 0)
    assert not mmcv2.assert_params_all_zeros(demo_module)
...
def test_check_python_script(capsys):
    mmcv2.utils.check_python_script("./tests/data/scripts/hello.py zz")
    captured = capsys.readouterr().out
    assert captured == "hello zz!\n"
    with pytest.warns(UserWarning, match="I have a secret!"):
        mmcv2.utils.check_python_script("./tests/data/scripts/hello.py agent")
    captured = capsys.readouterr().out
    assert captured == "hello agent!\n"
    # Make sure that wrong cmd raises an error
    with pytest.raises(SystemExit):
        mmcv2.utils.check_python_script("./tests/data/scripts/hello.py li zz")
