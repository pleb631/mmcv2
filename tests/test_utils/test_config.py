...
import pytest
import yaml
from mmcv2 import Config, ConfigDict, DictAction, dump, load
...
def test_construct():
    ...
    cfg_dict = {"item1": [1, 2], "item2": {"a": 0}, "item3": True, "item4": "test"}
    # test a.py
    ...
    cfg_dict = {"item1": "h.py", "item2": path, "item3": "abc_h"}
    cfg = Config(cfg_dict, filename=cfg_file)
    assert isinstance(cfg, Config)
    assert cfg.filename == cfg_file
    assert cfg.text == open(cfg_file).read()
    ...
    cfg_dict = {"item1": osp.join(osp.dirname(__file__), "data", "config")}
    cfg = Config(cfg_dict, filename=cfg_file)
    assert isinstance(cfg, Config)
    assert cfg.filename == cfg_file
    assert cfg.text == open(cfg_file).read()
...
def test_merge_from_multiple_bases():
    ...
    assert cfg.item5 == {"a": 0, "b": 1}
    assert cfg.item6 == [{"a": 0}, {"b": 1}]
    assert cfg.item7 == {"a": [0, 1, 2], "b": {"c": [3.1, 4.2, 5.3]}}
...
def test_merge_delete():
    ...
    cfg = Config.fromfile(cfg_file)
    # cfg.field
    assert cfg.item1 == {"a": 0}
    assert cfg.item2 == {"a": 0, "b": 0}
...
def test_merge_intermediate_variable():
    ...
    assert cfg.item3 is True
    assert cfg.item4 == "test"
    assert cfg.item_cfg == {"b": 2}
    assert cfg.item5 == {"cfg": {"b": 1}}
    assert cfg.item6 == {"cfg": {"b": 2}}
    ...
    def compare_json_cfg(ori_cfg, dumped_json_cfg):
        for key, value in ori_cfg.items():
            ...
            if not isinstance(value, dict):
                assert value == dumped_json_cfg[str(key)]
            else:
...
def test_dict_action():
    ...
    assert cfg.item2 == {"a": 1, "b": 0.1, "c": "x"}
    assert cfg.item3 is False
