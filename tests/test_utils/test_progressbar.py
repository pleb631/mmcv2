...
class TestProgressBar:
    def test_start(self):
        ...
        prog_bar = mmcv2.ProgressBar(bar_width=bar_width, file=out)
        assert out.getvalue() == "completed: 0, elapsed: 0s"
        reset_string_io(out)
        prog_bar = mmcv2.ProgressBar(bar_width=bar_width, start=False, file=out)
        assert out.getvalue() == ""
        reset_string_io(out)
        prog_bar.start()
        assert out.getvalue() == "completed: 0, elapsed: 0s"
        # with total task num
        reset_string_io(out)
        prog_bar = mmcv2.ProgressBar(10, bar_width=bar_width, file=out)
        assert out.getvalue() == f"[{' ' * bar_width}] 0/10, elapsed: 0s, ETA:"
        reset_string_io(out)
        prog_bar = mmcv2.ProgressBar(10, bar_width=bar_width, start=False, file=out)
        assert out.getvalue() == ""
        reset_string_io(out)
        prog_bar.start()
        assert out.getvalue() == f"[{' ' * bar_width}] 0/10, elapsed: 0s, ETA:"
...
def test_track_progress_list():
    out = StringIO()
    ret = mmcv2.track_progress(sleep_1s, [1, 2, 3], bar_width=3, file=out)
    assert out.getvalue() == (
...
def test_track_progress_iterator():
    out = StringIO()
    ret = mmcv2.track_progress(sleep_1s, ((i for i in [1, 2, 3]), 3), bar_width=3, file=out)
    assert out.getvalue() == (
        "[   ] 0/3, elapsed: 0s, ETA:\r[>  ] 1/3, 1.0 task/s, elapsed: 1s, ETA:     2s\r[>> ] 2/3, 1.0 task/s, elapsed: 2s, ETA:     1s\r[>>>] 3/3, 1.0 task/s, elapsed: 3s, ETA:     0s\n"
    )
    assert ret == [1, 2, 3]
...
def test_track_iter_progress():
    out = StringIO()
    ret = []
    for num in mmcv2.track_iter_progress([1, 2, 3], bar_width=3, file=out):
...
def test_track_enum_progress():
    ...
    ret = []
    count = []
    for i, num in enumerate(mmcv2.track_iter_progress([1, 2, 3], bar_width=3, file=out)):
        ret.append(sleep_1s(num))
        count.append(i)
    assert out.getvalue() == (
        "[   ] 0/3, elapsed: 0s, ETA:\r[>  ] 1/3, 1.0 task/s, elapsed: 1s, ETA:     2s\r[>> ] 2/3, 1.0 task/s, elapsed: 2s, ETA:     1s\r[>>>] 3/3, 1.0 task/s, elapsed: 3s, ETA:     0s\n"
...
def test_track_parallel_progress_list():
    out = StringIO()
    results = mmcv2.track_parallel_progress(sleep_1s, [1, 2, 3, 4], 2, bar_width=4, file=out)
...
def test_track_parallel_progress_iterator():
    out = StringIO()
    results = mmcv2.track_parallel_progress(sleep_1s, ((i for i in [1, 2, 3, 4]), 4), 2, bar_width=4, file=out)
    # The following cannot pass CI on Github Action
    # assert out.getvalue() == (
    #     '[    ] 0/4, elapsed: 0s, ETA:'
    #     '\r[>   ] 1/4, 1.0 task/s, elapsed: 1s, ETA:     3s'
