...
import numpy as np
import pytest
from mmcv2.image.colorspace import _convert_input_type_range, _convert_output_type_range
from numpy.testing import assert_array_almost_equal, assert_array_equal


def test_bgr2gray():
    in_img = np.random.rand(10, 10, 3).astype(np.float32)
    out_img = mmcv2.bgr2gray(in_img)
    computed_gray = in_img[:, :, 0] * 0.114 + in_img[:, :, 1] * 0.587 + in_img[:, :, 2] * 0.299
    assert_array_almost_equal(out_img, computed_gray, decimal=4)
    out_img_3d = mmcv2.bgr2gray(in_img, True)
...
def test_rgb2gray():
    in_img = np.random.rand(10, 10, 3).astype(np.float32)
    out_img = mmcv2.rgb2gray(in_img)
    computed_gray = in_img[:, :, 0] * 0.299 + in_img[:, :, 1] * 0.587 + in_img[:, :, 2] * 0.114
    assert_array_almost_equal(out_img, computed_gray, decimal=4)
    out_img_3d = mmcv2.rgb2gray(in_img, True)
    assert out_img_3d.shape == (10, 10, 1)
    assert_array_almost_equal(out_img_3d[..., 0], out_img, decimal=4)
...
def test_rgb2ycbcr():
    with pytest.raises(TypeError):
        # The img type should be np.float32 or np.uint8
        in_img = np.random.rand(10, 10, 3).astype(np.uint64)
        mmcv2.rgb2ycbcr(in_img)

    # float32
    in_img = np.random.rand(10, 10, 3).astype(np.float32)
    out_img = mmcv2.rgb2ycbcr(in_img)
    ...
    # y_only=True
    out_img = mmcv2.rgb2ycbcr(in_img, y_only=True)
...
def test_bgr2ycbcr():
    # float32
    in_img = np.random.rand(10, 10, 3).astype(np.float32)
    out_img = mmcv2.bgr2ycbcr(in_img)
    computed_ycbcr = np.empty_like(in_img)
    for i in range(in_img.shape[0]):
        for j in range(in_img.shape[1]):
            b, g, r = in_img[i, j]
    ...
    # y_only=True
    in_img = np.random.rand(10, 10, 3).astype(np.float32)
    out_img = mmcv2.bgr2ycbcr(in_img, y_only=True)
    computed_y = np.empty_like(out_img, dtype=out_img.dtype)
    for i in range(in_img.shape[0]):
        for j in range(in_img.shape[1]):
            b, g, r = in_img[i, j]
...
def test_ycbcr2rgb():
    with pytest.raises(TypeError):
        # The img type should be np.float32 or np.uint8
        in_img = np.random.rand(10, 10, 3).astype(np.uint64)
        mmcv2.ycbcr2rgb(in_img)

    # float32
    in_img = np.random.rand(10, 10, 3).astype(np.float32)
    out_img = mmcv2.ycbcr2rgb(in_img)
    ...
    for i in range(in_img.shape[0]):
        for j in range(in_img.shape[1]):
            ...
            g = 135.576 + y * 0.00456621 * 255 - cb * 0.00153632 * 255 - cr * 0.00318811 * 255
...
def test_ycbcr2bgr():
    # float32
    in_img = np.random.rand(10, 10, 3).astype(np.float32)
    out_img = mmcv2.ycbcr2bgr(in_img)
...
def test_imconvert(src, dst, ref):
    ...
    assert_array_equal(mmcv2.imconvert(img, src, dst), cv2.cvtColor(img, ref))
