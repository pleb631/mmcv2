...
# yapf: enable
from mmcv2.runner.hooks.hook import HOOKS, Hook
from mmcv2.runner.hooks.lr_updater import (
...
from mmcv2.utils import build_from_cfg
from torch import nn
from torch.nn.init import constant_
from torch.utils.data import DataLoader


class RecordingLoggerHook(LoggerHook):
    """Capture scheduler values without an external logging service."""
    ...
    def log(self, runner) -> None:
        ...
        if tags:
            step = self.get_epoch(runner) if self.get_mode(runner) == "val" and self.by_epoch else self.get_iter(runner)
            self.writer.add_scalars(self.get_mode(runner), tags, step)


def assert_scalar_calls(writer, expected_calls):
    """Check selected scalar writes while tolerating floating-point noise."""
    actual_by_step = {(item.args[0], item.args[2]): item.args[1] for item in writer.add_scalars.call_args_list}
    for expected in expected_calls:
        mode, scalars, step = expected.args
        key = (mode, step)
        assert key in actual_by_step
        assert actual_by_step[key] == pytest.approx(scalars)


def test_logger_backends_from_config():
    names = {name for name in HOOKS.module_dict if name.endswith("LoggerHook")}
    assert names == {
        "TextLoggerHook",
        "TensorboardLoggerHook",
        "WandbLoggerHook",
    ...
    with patch.dict(sys.modules, {"wandb": MagicMock()}):
        for hook_type in (TextLoggerHook, TensorboardLoggerHook, WandbLoggerHook):
            hook = build_from_cfg({"type": hook_type.__name__}, HOOKS)
            assert isinstance(hook, hook_type)
...
def test_optimizerhook():
    ...
    optimizer_hook = OptimizerHook({"max_norm": 2}, detect_anomalous_params=True)
...
def test_custom_hook():
    ...
    custom_hooks_cfg = [
        {"type": "ToyHook", "priority": 51, "info": 51},
        {"type": "ToyHook", "priority": 49, "info": 49},
    ]
    ...
    custom_hooks_cfg = [{"type": "ToyHook", "priority": rank, "info": rank} for rank in random_priority_ranks]
    runner.register_custom_hooks(custom_hooks_cfg)
    ...
    custom_hooks_cfg = [
        {"type": "ToyHook", "priority": 1, "info": "custom 1"},
        {"type": "ToyHook", "priority": "NORMAL", "info": "custom normal"},
        {"type": "ToyHook", "priority": 89, "info": "custom 89"},
    ]
    runner.register_training_hooks(
        lr_config=ToyHook("lr"),
        optimizer_config=ToyHook("optimizer"),
        checkpoint_config=ToyHook("checkpoint"),
        log_config={"interval": 1, "hooks": [{"type": "ToyHook", "info": "log"}]},
...
def test_sync_buffers_hook():
    loader = DataLoader(torch.ones((5, 2)))
    runner = _build_demo_runner()
    runner.register_hook_from_cfg({"type": "SyncBuffersHook"})
    runner.run([loader, loader], [("train", 1), ("val", 1)])
    shutil.rmtree(runner.work_dir)


@pytest.mark.parametrize(
    "multi_optimizers, max_iters, gamma, cyclic_times",
...
def test_momentum_runner_hook(multi_optimizers, max_iters, gamma, cyclic_times):
    ...
    runner.register_hook_from_cfg(hook_cfg)
    runner.register_hook_from_cfg({"type": "IterTimerHook"})

    # record scheduler metrics
    hook = RecordingLoggerHook(interval=1)
    ...
    if multi_optimizers:
        ...
        calls = [
            call("train", {"learning_rate": 0.01999999999999999, "momentum": 0.95}, 1),
            call("train", {"learning_rate": 0.11, "momentum": 0.85}, 3),
            call("train", {"learning_rate": 0.1879422863405995, "momentum": 0.95}, 6),
            call("train", {"learning_rate": 0.11000000000000001, "momentum": 0.9}, 8),
        ]
    assert_scalar_calls(hook.writer, calls)
    ...
    if multi_optimizers:
        ...
        calls = [
            call("train", {"learning_rate": 0.02, "momentum": 1.9}, 1),
            call("train", {"learning_rate": 0.02, "momentum": 1.9}, 5),
            call("train", {"learning_rate": 0.02, "momentum": 0.95}, 10),
        ]
    ...
    if multi_optimizers:
        ...
        calls = [
            call("train", {"learning_rate": 0.02, "momentum": 1.9}, 1),
            call("train", {"learning_rate": 0.02, "momentum": 1.3571428571428572}, 3),
            call("train", {"learning_rate": 0.02, "momentum": 0.95}, 10),
        ]
    ...
    if multi_optimizers:
        ...
        calls = [
            call("train", {"learning_rate": 0.02, "momentum": 1.9}, 1),
            call("train", {"learning_rate": 0.02, "momentum": 1.4399307381848783}, 3),
            call("train", {"learning_rate": 0.02, "momentum": 0.95}, 10),
        ]
...
def test_cosine_runner_hook(multi_optimizers):
    ...
    if multi_optimizers:
        ...
        calls = [
            call("train", {"learning_rate": 0.02, "momentum": 0.95}, 1),
            call("train", {"learning_rate": 0.01, "momentum": 0.97}, 6),
            call(
                "train",
                {
    ...
    if multi_optimizers:
        ...
        if by_epoch:
            calls = [
                call(
                    "train",
                    {"learning_rate": 0.018000000000000002, "momentum": 0.95},
                    1,
                ),
                call("train", {"learning_rate": 0.02, "momentum": 0.95}, 11),
                call(
                    "train",
                    {"learning_rate": 0.018090169943749474, "momentum": 0.95},
                    61,
                ),
                call(
                    "train",
                    {"learning_rate": 0.0019098300562505265, "momentum": 0.95},
                    100,
                ),
            ]
        else:
            calls = [
                ...
                call("train", {"learning_rate": 0.02, "momentum": 0.95}, 6),
                call(
                    "train",
                    {"learning_rate": 0.018090169943749474, "momentum": 0.95},
                    7,
...
def test_one_cycle_runner_hook(multi_optimizers, max_iters):
    ...
    if multi_optimizers:
        ...
        calls = [
            call("train", {"learning_rate": 0.0003999999999999993, "momentum": 0.95}, 1),
            call(
                "train",
                {"learning_rate": 0.00904508879153485, "momentum": 0.8595491502812526},
                6,
            ),
            call("train", {"learning_rate": 4e-08, "momentum": 0.95}, 10),
        ]
    assert_scalar_calls(hook.writer, calls)
...
def test_format_lr_param_accepts_numpy_scalar():
    parameter = nn.Parameter(torch.ones(1))
    optimizer = torch.optim.SGD([parameter], lr=0.1)

    assert format_param("SGD", optimizer, np.float32(0.2)) == pytest.approx([0.2])
...
def test_cosine_restart_lr_update_hook(multi_optimizers):
    ...
    if multi_optimizers:
        ...
        calls = [
            call("train", {"learning_rate": 0.01, "momentum": 0.95}, 1),
            call("train", {"learning_rate": 0.01, "momentum": 0.95}, 6),
            call("train", {"learning_rate": 0.0009549150281252633, "momentum": 0.95}, 10),
        ]
    assert_scalar_calls(hook.writer, calls)
...
def test_step_runner_hook(multi_optimizers):
    ...
    if multi_optimizers:
        ...
        calls = [
            call("train", {"learning_rate": 0.02, "momentum": 0.95}, 1),
            call("train", {"learning_rate": 0.01, "momentum": 0.475}, 6),
            call("train", {"learning_rate": 0.0025, "momentum": 0.11875}, 16),
            call("train", {"learning_rate": 0.00125, "momentum": 0.059375}, 21),
            call("train", {"learning_rate": 0.001, "momentum": 0.05}, 26),
            call("train", {"learning_rate": 0.001, "momentum": 0.05}, 30),
        ]
    assert_scalar_calls(hook.writer, calls)
    ...
    if multi_optimizers:
        ...
        calls = [
            call("train", {"learning_rate": 0.02, "momentum": 0.95}, 1),
            call("train", {"learning_rate": 0.002, "momentum": 0.095}, 5),
            call(
                "train",
                {
...
def test_cyclic_lr_update_hook(multi_optimizers, max_iters, gamma, cyclic_times):
    ...
    if multi_optimizers:
        ...
        calls = [
            call("train", {"learning_rate": 0.02, "momentum": 0.95}, 1),
            call("train", {"learning_rate": 0.11, "momentum": 0.95}, 4),
            call("train", {"learning_rate": 0.065, "momentum": 0.95}, 6),
            call("train", {"learning_rate": 0.11, "momentum": 0.95}, 7),
        ]
    assert_scalar_calls(hook.writer, calls)
...
def test_wandb_hook():
    ...
    runner = _build_demo_runner()
    hook = WandbLoggerHook(log_artifact=True, define_metric_cfg={"val/loss": "min"})
    loader = DataLoader(torch.ones((5, 2)))
...
def _build_demo_runner_without_hook(
    runner_type="EpochBasedRunner", max_epochs=1, max_iters=None, multi_optimizers=False
):
...
def _build_demo_runner(runner_type="EpochBasedRunner", max_epochs=1, max_iters=None, multi_optimizers=False):
    ...
    runner.register_checkpoint_hook({"interval": 1})
...
def test_runner_with_revise_keys():
    ...
    runner.model = pmodel
    state_dict = runner.load_checkpoint(checkpoint_path, revise_keys=[(r"^", "backbone.")])
...
def test_gradient_cumulative_optimizer_hook():
    ...
    def build_toy_runner(config=None):
        if config is None:
            config = {"type": "EpochBasedRunner", "max_epochs": 3}
        model = ToyModel()
    ...
    optimizer_hook = GradientCumulativeOptimizerHook(grad_clip={"max_norm": 0.2}, cumulative_iters=3)
    ...
    optimizer_hook = OptimizerHook(grad_clip={"max_norm": 0.2})
    ...
    # optimize with cumulative_iters
    loader_1 = DataLoader(data, batch_size=1)
    runner_1 = build_toy_runner({"type": "IterBasedRunner", "max_iters": 8})
    optimizer_hook = GradientCumulativeOptimizerHook(grad_clip={"max_norm": 0.2}, cumulative_iters=3)
    runner_1.register_hook(optimizer_hook)
    runner_1.run([loader_1], [("train", 1)])
    ...
    runner_2 = build_toy_runner({"type": "IterBasedRunner", "max_iters": 3})
    optimizer_hook = OptimizerHook(grad_clip={"max_norm": 0.2})
    runner_2.register_hook(optimizer_hook)
    ...
    runner = build_toy_runner({"type": "EpochBasedRunner", "max_epochs": 2})
    ...
    runner = build_toy_runner({"type": "IterBasedRunner", "max_iters": 6})
    assert calc_loss_factors(runner) == [3] * 6
    runner = build_toy_runner({"type": "IterBasedRunner", "max_iters": 7})
    assert calc_loss_factors(runner) == [3] * 6 + [1]
    runner = build_toy_runner({"type": "IterBasedRunner", "max_iters": 8})
    assert calc_loss_factors(runner) == [3] * 6 + [2, 2]
    runner = build_toy_runner({"type": "IterBasedRunner", "max_iters": 6})
