...
import pytest
import torch
from mmcv2.runner import RUNNERS, EpochBasedRunner, IterBasedRunner, build_runner
from mmcv2.runner.hooks import IterTimerHook
...
def test_build_runner():
    ...
    cfg = {"type": "EpochBasedRunner", "max_epochs": 1}
    runner = build_runner(cfg, default_args=default_args)
    assert runner._max_epochs == 1
    cfg = {"type": "IterBasedRunner", "max_iters": 1}
    runner = build_runner(cfg, default_args=default_args)
    assert runner._max_iters == 1

    with pytest.raises(ValueError, match="Only one of"):
        cfg = {"type": "IterBasedRunner", "max_epochs": 1, "max_iters": 1}
        runner = build_runner(cfg, default_args=default_args)


@pytest.mark.parametrize(
    "runner_type, limit",
    [(EpochBasedRunner, {"max_epochs": 1}), (IterBasedRunner, {"max_iters": 1})],
)
def test_runner_rejects_non_advancing_workflow(runner_type, limit):
    runner = runner_type(Model(), logger=logging.getLogger("workflow"), **limit)
    loader = torch.utils.data.DataLoader(torch.ones(1, 1))
    with pytest.raises(ValueError, match="train phase"):
        runner.run([loader], [("val", 1)])
    with pytest.raises(ValueError, match="positive count"):
        runner.run([loader], [("train", 0)])
...
def test_epoch_based_runner(runner_class):
    with pytest.raises(TypeError):
        ...
        model = Model()
        optimizers = {"optim1": torch.optim.Adam(), "optim2": "NotAOptimizer"}
