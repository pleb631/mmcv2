import pytest
import torch
from mmcv2.runner import OPTIMIZER_BUILDERS, DefaultOptimizerConstructor
from mmcv2.runner.optimizer import build_optimizer, build_optimizer_constructor
from mmcv2.runner.optimizer.builder import TORCH_OPTIMIZERS
...
def test_default_optimizer_constructor():
    ...
    with pytest.raises(ValueError):
        ...
        optimizer_cfg = {"lr": 0.0001, "weight_decay": None}
        paramwise_cfg = {"bias_decay_mult": 1, "norm_decay_mult": 1}
        optim_constructor = DefaultOptimizerConstructor(optimizer_cfg, paramwise_cfg)
        optim_constructor(model)
    ...
    model = ExampleModel()
    optimizer_cfg = {"type": "Rprop", "lr": base_lr}
    paramwise_cfg = {"bias_lr_mult": 2}
    optim_constructor = DefaultOptimizerConstructor(optimizer_cfg, paramwise_cfg)
    optimizer = optim_constructor(model)
    ...
    paramwise_cfg = {
        "custom_keys": {
            "param1": {"lr_mult": 10},
            "sub": {"lr_mult": 0.1, "decay_mult": 0},
            "sub.gn": {"lr_mult": 0.01},
            "non_exist_key": {"lr_mult": 0.0},
        },
        "norm_decay_mult": 0.5,
    }
    ...
    with pytest.raises(TypeError):
        ...
        paramwise_cfg_ = {"custom_keys": [0.1, 0.0001]}
    ...
    with pytest.raises(ValueError):
        ...
        # specified
        optimizer_cfg_ = {"type": "SGD", "lr": 0.01}
        paramwise_cfg_ = {"custom_keys": {".backbone": {"decay_mult": 0.5}}}
    ...
    model = ExampleModel()
    optimizer_cfg = {"type": "SGD", "lr": base_lr, "momentum": momentum}
    paramwise_cfg = {"custom_keys": {"param1": {"lr_mult": 10}}}
...
def test_build_optimizer_constructor():
    ...
    from mmcv2.runner import OPTIMIZERS
